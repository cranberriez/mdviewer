...
export function useInternalDrag({ activeRootPath, onDrop }: UseInternalDragOptions): InternalDragController {
  ...
  const updateActiveState = useCallback((x: number, y: number, target: DropZone | null, items: DragItem[], mode: DropMode) => {
    setState({
      active: true,
      origin: "internal",
      items,
  ...
  const escalateToNative = useCallback(
    (current: PendingDrag) => {
