...
export function OnboardingView({
  ...
  function removeFolder(path: string) {
    setStarterFolders((current) => current.filter((entry) => comparable(entry.path) !== comparable(path)));
  }

  function complete() {
    onComplete({
      name: name.trim(),
      starterFolders: homeOverride
        ? [homeOverride, ...starterFolders]
        : starterFolders,
      viewMode,
    });
  }
        ...
        <div className="ob-field">
          ...
          <div className="ob-folder-list">
            <div className="ob-folder-row">
              ...
              <button type="button" className="ob-row-act" title="Choose a different folder" onClick={() => void repointHome()}>
                <FolderOpen size={14} />
              </button>
            </div>
            {starterFolders.map((folder) => (
              <div key={folder.path} className="ob-folder-row">
                ...
                <button type="button" className="ob-row-act" title="Remove" onClick={() => removeFolder(folder.path)}>
                  <Minus size={14} />
                </button>
              </div>
            ))}
