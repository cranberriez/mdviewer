...
function $toggleHeading() {
	...
	$setBlocksType(selection, () => (isH1 ? $createParagraphNode() : $createHeadingNode('h1')));
}
...
function $toggleQuote() {
	...
	$setBlocksType(selection, () => (isQuote ? $createParagraphNode() : $createQuoteNode()));
}
...
function $insertImage() {
	const selection = $getSelection();
	const alt = $isRangeSelection(selection) ? selection.getTextContent() || 'alt' : 'alt';
 ...
 */
export function applyMarkdownActionToEditor(editor: LexicalEditor, action: MarkdownAction): void {
	editor.focus();
	editor.update(() => {
		switch (action) {
			case 'bold':
			...
			case 'strike': {
				const format: TextFormatType =
					action === 'bold' ? 'bold' : action === 'italic' ? 'italic' : 'strikethrough';
