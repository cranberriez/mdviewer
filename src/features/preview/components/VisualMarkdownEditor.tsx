...
function indentListItem(item: HTMLElement) {
	...
	if (!(previous instanceof HTMLElement) || previous.tagName.toLowerCase() !== 'li') {
		// No preceding sibling to nest under; can't indent further.
		return false;
	}
...
function outdentListItem(item: HTMLElement, root: HTMLElement) {
	...
	// Move the item (and any following siblings) out one level.
...
function insertTabAtSelection(editor: HTMLElement, outdent: boolean) {
	...
	if (listItem) {
		...
		// Even if nesting couldn't change, swallow Tab so focus doesn't move.
		return true;
	}

	// Plain text / empty block: outdent removes a leading indent, Tab inserts one.
	if (outdent) {
...
function removeLeadingIndentAtCaret(editor: HTMLElement, range: Range, selection: Selection) {
	...
	// Remove a leading tab span if present.
	...
	// Keep caret stable inside the block.
	const caret = document.createRange();
...
function replaceBlockWithElement(block: HTMLElement, element: HTMLElement, selectedNode?: Node) {
